...
const navItems = [
  ...
  { id: 'discussions', label: 'Discussions', icon: <ChatAlt2Icon /> },
  { id: 'community', label: 'Community', icon: <GlobeIcon /> },
  { id: 'connect', label: 'Connect With A Professional', icon: <BriefcaseIcon /> },
  { id: 'podcasts', label: 'Podcasts', icon: <MicrophoneIcon /> },
  { id: 'webinars', label: 'Webinars', icon: <PresentationChartBarIcon /> },
  { id: 'resources', label: 'Research Resources', icon: <DocumentTextIcon /> },
  { id: 'events', label: 'Events', icon: <CalendarIcon /> },
] as const;

export const Sidebar: React.FC<SidebarProps> = ({ currentView, setView }) => {
  return (
    <aside className="w-64 bg-brand-dark text-white flex-shrink-0 flex flex-col">
      <div className="h-16 flex items-center justify-center text-2xl font-bold border-b border-brand-dark/50">
        FamilyNation
      </div>
